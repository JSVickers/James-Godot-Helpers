...
func calculate_velocity_change(current_velocity: Vector3, direction: Vector3, movement_threshold: float, max_speed: float, min_speed: float, acceleration: float, deceleration: float, delta: float, acceleration_curve: Curve = null) -> Vector3:
	...
	var decelerating := angle > 45 or target_velocity == Vector3.ZERO
	if decelerating:
		acceleration_rate = deceleration
	elif acceleration_curve != null:
		var speed_progress = clampf(current_velocity.length() / max_speed, min_speed / max_speed, 1)
